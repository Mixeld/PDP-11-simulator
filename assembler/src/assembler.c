...
static void pass1(const char *filename) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (is_ignored_directive(pl.mnemonic)) continue;
        ...
        if (portable_strcasecmp(pl.mnemonic, ".EVEN") == 0) {
            if (LC & 1) {
                LC++;
                if (!quiet && verbose) printf("  .EVEN -> выравнивание\n");
            }
            continue;
        }
        if (portable_strcasecmp(pl.mnemonic, ".ODD") == 0) {
            if (!(LC & 1)) {
                LC++;
                if (!quiet && verbose) printf("  .ODD -> выравнивание\n");
            }
            continue;
        }
        if (portable_strcasecmp(pl.mnemonic, ".WORD") == 0) {
            if (LC & 1) LC++;
            int old_lc = LC;
            LC += word_count_bytes(pl.operands_raw);
            if (!quiet && verbose) {
        ...
        if (portable_strcasecmp(pl.mnemonic, ".BYTE") == 0) {
            int old_lc = LC;
            LC += byte_count_bytes(pl.operands_raw);
            if (!quiet && verbose) {
        ...
        if (portable_strcasecmp(pl.mnemonic, ".ASCII") == 0 ||
            ...
            int old_lc = LC;
            LC += (uint16_t)process_string(str, 0, 0);
            if (portable_strcasecmp(pl.mnemonic, ".ASCIZ") == 0) LC++;
            if (!quiet && verbose) {
                printf("  %s: %d байт, адрес %06o -> %06o\n", 
                       pl.mnemonic, LC - old_lc, old_lc, LC);
            }
            continue;
        }
        ...
        split_operands(pl.operands_raw, ops, 4);

        switch (instr->type) {
        case SINGLE_OP:
        case EIS_OP:
            parse_operand(ops[0], &oi);
            if (oi.has_next_word) LC += 2;
            break;
        case DOUBLE_OP:
            parse_operand(ops[0], &oi);
            if (oi.has_next_word) LC += 2;
        ...
        if (!quiet && verbose) {
            printf("  %s: %d байт, адрес %06o -> %06o\n", 
                   instr->mnemonic, LC - old_lc, old_lc, LC);
        }
    }
...
static void export_tape(const char *out_filename, uint16_t start_addr) {
    ...
    while (pos < MEMORY_SIZE) {
        if (!memory_used[pos]) { pos++; continue; }

        uint32_t end = pos;
        while (end < MEMORY_SIZE && memory_used[end] &&
               (end - pos) < MAX_BLOCK_DATA)
            end++;

        write_tape_block(f, (uint16_t)pos, &memory[pos], (int)(end - pos));
        pos = end;
    }

    write_tape_block(f, start_addr, NULL, 0);
